...
if (!this.PriceRegexes) {
    ...
    const PriceRegexes = (function() {
        ...
        const makePriceRegexes = (aRegex1, aRegex2) => {

            // TODO add possibility to switch true or false
            const isPoundCurrency = false;
            const pounds = isPoundCurrency ? "|pounds?" : "";
            const begin = "(?:^|\\s|\\()";
            ...
            //const end = "(?!\\s)";
            const makeRegex1 = (reg, mult) => {
//                console.log(begin + reg + space + value + space + mult);
                let completeRegString;
                if (mult) {
                    completeRegString = begin + reg + space + value + space + mult;
                }
                else {
                    completeRegString = begin + reg + space + value + space + "()?";
                }
//                console.log(completeRegString);
                return RegExp(completeRegString, "g");
            };

            const makeRegex2 = (reg, mult) => {
//                console(value + space + mult + space + reg + end);
                let completeRegString;
                if (mult) {
                    completeRegString = value + space + mult + space + reg + end;
                }
                else {
                    completeRegString = value + space + "()?" + space + reg + end;
                }
//                console.log(completeRegString);
                return RegExp(completeRegString, "g");
            };
